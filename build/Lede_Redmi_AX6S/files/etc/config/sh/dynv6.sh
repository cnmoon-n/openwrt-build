...
#ip=`ip -6 addr show dev wan |grep 'scope global'|grep -v deprecated|awk -F '/|inet6 ' 'NR==1{print $2;}' 2>&1` || die "$ip"
ip=`ip -6 addr show dev wan |grep '240'|grep -v deprecated|awk -F '/|inet6 ' 'NR==1{print $2;}' 2>&1` || die "$ip"
...
curl "https://dynv6.com/api/update?hostname=eyenclede.dynv6.net&ipv6=$ip&token=876xxUQDk5yvkK4WL74izEeSbiCn7k" -k > /etc/config/sh/dynv6.log
