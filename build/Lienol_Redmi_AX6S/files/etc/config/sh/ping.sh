...
ret=$?
if [ $ret -eq 0 ]
then printf "$prefix is good \n" > /etc/config/sh/ping.log&&ping www.163.com -w 1 -4
else printf "$prefix is down \n"> /etc/config/sh/ping123.1.log&&ping 192.168.123.1 -w 1 >> /etc/config/sh/ipv4error.log&&chmod -R 0777 /mnt/sdc1&&/etc/init.d/AdGuardHome restart
fi
return 0
}
