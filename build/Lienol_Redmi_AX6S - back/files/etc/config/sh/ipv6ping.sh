...
ret=$?
if [ $ret -eq 0 ]
then printf "$prefix is good \n" > /etc/config/sh/pingipv6.log&&ping www.163.com -w 1
else printf "$prefix is down \n"> /etc/config/sh/ping123.1.log&&ping 192.168.123.1 -w 1 >> /etc/config/sh/ipv6error.log&&/etc/init.d/openclash restart
fi
return 0
}
